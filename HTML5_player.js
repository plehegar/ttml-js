...
/*
  This comes originally from
    http://www.w3.org/2011/11/HTML5_player.js
...
HTML5Caption_convertDFXP2HTMLAttributes = function(dfxpElement, htmlElement, hasOrigin, top, left) {
    ...
    if (dfxpElement.hasAttributeNS(XHTML_NS, "style")) {
	htmlElement.style.cssText = dfxpElement.getAttributeNS(XHTML_NS, "style");
    }
    v = dfxpElement.getAttribute("region");
    if (dfxpElement.hasAttribute("region")) {        
	var dfxpElementRef = dfxpElement.ownerDocument.getElementById(v);
	    
	if (dfxpElementRef == null && !!dfxpElement.ownerDocument.getElementsByTagNameNS) {
	...
	if (dfxpElementRef != null) {
	    ...
	    if (!!dfxpElementRef.getElementsByTagNameNS) {
    ...
    if (dfxpElement.hasAttribute("style")) { 
    ...
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "backgroundColor")) {
	htmlElement.style.setProperty("background-color", dfxpElement.getAttributeNS(DFXP_NS_Style, "backgroundColor"), "");
    }
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "color")) {
	htmlElement.style.setProperty("color", dfxpElement.getAttributeNS(DFXP_NS_Style, "color"), "");
    }
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "direction")) {
	htmlElement.style.setProperty("direction", dfxpElement.getAttributeNS(DFXP_NS_Style, "direction"), "");
    }
	...
	if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "display") || !htmlElement.df_displayValue) {
	    htmlElement.style.setProperty("display", v, "");
	    htmlElement.df_displayValue = v;
	}
    }
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "fontFamily")) {
	htmlElement.style.setProperty("font-family", dfxpElement.getAttributeNS(DFXP_NS_Style, "fontFamily"), "");
    }
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "fontSize")) {
	htmlElement.style.setProperty("font-size", dfxpElement.getAttributeNS(DFXP_NS_Style, "fontSize"), "");
    }
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "fontStyle")) {
	htmlElement.style.setProperty("font-style", dfxpElement.getAttributeNS(DFXP_NS_Style, "fontStyle"), "");
    }
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "fontWeight")) {
	htmlElement.style.setProperty("font-weight", dfxpElement.getAttributeNS(DFXP_NS_Style, "fontWeight"), "");
    }
    v = dfxpElement.getAttributeNS(DFXP_NS_Style, "lineHeight");
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "lineHeight")) {
	if (v == "normal") {
    ...
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "opacity")) {
	htmlElement.style.setProperty("opacity", v, "");
    }
    v = dfxpElement.getAttributeNS(DFXP_NS_Style, "overflow");
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "overflow")) {
	if (v == "dynamic") {	    
    ...
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "extent")) {
	if (v != "auto") {
	    var p = v.split(' ');
	    if (p.length = 2) {
    ...
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "origin")) {
	if (v != "auto" && v != "inherit") {
	    var p = v.split(' ');
	    if (p.length = 2) {
    ...
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "padding")) {
	htmlElement.style.setProperty("padding", v, "");
    }
    v = dfxpElement.getAttributeNS(DFXP_NS_Style, "textAlign");
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "textAlign")) {
    ...
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "textDecoration")) {
    ...
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "unicodeBidi")) {
	if (v == "bidiOverride") {
	    v = "bidi-override";
	}
    ...
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "visibility")) {
	htmlElement.style.setProperty("visibility", v, "");
    }
    v = dfxpElement.getAttribute("xml:space");
    if (dfxpElement.hasAttribute("xml:space")) {
	if (v == "preserve") {
	    v = "pre";
	} else {
	    v = "normal";
    ...
    if (dfxpElement.hasAttributeNS(DFXP_NS_Style, "wrapOption")) {
	if (v == "noWrap") {
...
HTML5Caption_playDFXP = function(video, dfxpDocument) {    
    ...
    if (typeof dfxpDocument.getElementsByTagNameNS == "undefined") {
	dfxpDocument.bodyElement = dfxpDocument.documentElement.childNodes.item(1);
    } else {
    ...
    captionsElement.appendChild(mainDiv);
...
function init_captions() {
    var textElements = document.getElementsByTagName("track");
    var srtElement = null;
    var dfxpElement = null;

    for (var i = 0; i < textElements.length; i++) {
	var e = textElements.item(i);
	// @@ @type isn't defined in HTML5
	var type = e.getAttribute("type");
	// @@ should take into account @srclang
